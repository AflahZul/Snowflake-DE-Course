...
RETURNS NUMBER -- Returns the number of rows affected (though often just returns 1 for success)
LANGUAGE SQL
AS
$$
BEGIN
    -- The UPDATE statement is executed as the primary action
    UPDATE tasty_bytes_clone.raw_pos.menu
    SET SALE_PRICE_USD = SALE_PRICE_USD - 1
    WHERE MENU_ITEM_NAME = 'Mango Sticky Rice';

    -- Note: Snowflake Scripting returns the last executed SQL statement result count implicitly, 
    -- but you often return a fixed value for simplicity in a stored procedure.
END;
$$
;
